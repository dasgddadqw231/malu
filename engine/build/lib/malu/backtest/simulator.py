...
# Defense modules the simulator can faithfully replay
_SUPPORTED_DEFENSE = {"fixed_pct", "trailing_stop", "atr_stop", "trailing_atr", "swing_sr"}
# Filter modules the simulator can faithfully replay
_SUPPORTED_FILTERS = {"trend_filter"}
...
def _get_judgment_class(module_name: str) -> type[JudgmentModule]:
    ...
    # Fail loudly: silently falling back would validate a different strategy
    # than the one deployed live.
    if module_name not in registry:
        raise ValueError(
            f"backtester does not support judgment module '{module_name}' "
...
class BacktestSimulator:
    ...
    def __init__(self, config: BacktestConfig):
        ...
        if self.defense_module not in _SUPPORTED_DEFENSE:
            raise ValueError(
                f"backtester does not support defense module '{self.defense_module}' "
                f"(supported: {sorted(_SUPPORTED_DEFENSE)})"
            )
        if self.defense_module == "trailing_stop":  # live alias of fixed_pct
            self.defense_module = "fixed_pct"
        ...
        for f_cfg in filters_cfg:
            f_module = f_cfg.get("module", "")
            if f_module not in _SUPPORTED_FILTERS:
                raise ValueError(
                    f"backtester does not support filter module '{f_module}' "
                    f"(supported: {sorted(_SUPPORTED_FILTERS)})"
                )
            if f_module == "trend_filter":
