...
def _interval_minutes(interval: str) -> int:
    """Convert a Bybit kline interval string to minutes."""
    special = {"D": 1440, "W": 10080, "M": 43200}
    if interval in special:
        return special[interval]
    try:
        return int(interval)
    except ValueError:
        return 15
...
class Bot:
    ...
    def __init__(
        ...
        # Post-close cooldown in bars of the judgment interval (parity with backtester)
        cooldown_bars = int(sc.get("cooldown_bars", 0) or 0)
        interval = str(j_cfg.get("params", {}).get("interval", "15"))
        self.cooldown_seconds = cooldown_bars * _interval_minutes(interval) * 60
    ...
    async def kill(self) -> None:
        ...
        try:
            ...
            for pos in positions:
                await self.rate_limiter.acquire("order")
                resp = await self.client.close_position(
                    Category(self.config.category),
                    pos.symbol,
                    pos.side,
                    pos.size,
                )
                # Emergency path: record with unrealised estimate, skip pnl retries
                close_side = Side.SELL if pos.side == Side.BUY else Side.BUY
                await self._emit("kill_close", {
                    "reason": "kill switch",
                    "symbol": pos.symbol,
                    "side": str(close_side),
                    "qty": str(pos.size),
                    "order_link_id": resp.order_link_id or "",
                    "pnl": str(pos.unrealised_pnl or Decimal("0")),
                })
        except Exception as e:
    ...
    async def _fetch_realized_pnl(self, fallback: Decimal) -> Decimal:
        """Fetch realized PnL of the most recent close from the exchange.

        Bybit's closed-pnl record includes fees, unlike the unrealised snapshot
        taken just before closing. Retries briefly since the record can lag the
        close order by a moment; falls back to the unrealised estimate.
        """
        for attempt in range(3):
            try:
                await asyncio.sleep(1.0 if attempt else 0.5)
                await self.rate_limiter.acquire("position")
                records, _ = await self.client.get_closed_pnl(
                    Category(self.config.category), self.config.symbol, limit=1
                )
                if records:
                    return Decimal(records[0]["closedPnl"])
            except Exception as e:
                log.warning("closed_pnl_fetch_failed", bot_id=self.bot_id, error=str(e))
        return fallback

    async def _close_and_record(self, position, event: str, reason: str) -> None:
        """Close a position, record realized PnL, and emit a close event."""
        await self.rate_limiter.acquire("order")
        resp = await self.client.close_position(
            Category(self.config.category),
            position.symbol,
        ...
        pnl = await self._fetch_realized_pnl(position.unrealised_pnl or Decimal("0"))
    ...
    async def _run_cycle(self) -> None:
        ...
        if current_position:
            ...
            if current_position.liq_price > 0 and self.config.leverage > 1:
                ...
                if liq_dist_pct < Decimal("2"):
                    ...
                    await self._close_and_record(
                        current_position,
                        "liquidation_guard",
                        f"liquidation proximity: {liq_dist_pct:.2f}% from liq price",
                    )
                    return
            ...
            if defense_result.should_close:
                ...
                await self._close_and_record(
                    current_position, "defense_triggered", defense_result.reason
                )
                return
        ...
        # 3.5 Post-close cooldown (cooldown_bars, parity with backtester)
        if self.cooldown_seconds > 0 and self._last_close_ts is not None:
            elapsed = time.monotonic() - self._last_close_ts
            if elapsed < self.cooldown_seconds:
                remaining = int(self.cooldown_seconds - elapsed)
                self.status = BotStatus.WAITING
                await self._emit("status", {"phase": "cooldown", "remaining_sec": remaining})
                return
        ...
        try:
            ...
            if result.executed:
                ...
                await self._emit("trade", {
                    ...
                    "side": str(result.side) if result.side else "",
                    "qty": str(result.qty),
