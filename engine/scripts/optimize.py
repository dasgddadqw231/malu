...
3가지 전략(bb_rsi, macd_cross, ema_cross) × 다양한 파라미터 조합을
백테스트로 돌려서 수익 극대화되는 시그니처를 찾습니다.

데이터는 인터벌당 1회만 가져와서 재사용 → 수천 개 조합도 빠르게 처리.

Usage:
    cd engine
    .venv/bin/python -m scripts.optimize --symbol BTCUSDT --start 2025-01-01 --end 2025-04-01
    .venv/bin/python -m scripts.optimize --symbol ETHUSDT --intervals 5,15 --top 10
"""
...
async def main():
    parser = argparse.ArgumentParser(description="단타 전략 파라미터 최적화")
    parser.add_argument("--symbol", default="BTCUSDT")
    parser.add_argument("--start", default="2025-01-01", help="시작일 YYYY-MM-DD")
    parser.add_argument("--end", default="2025-04-01", help="종료일 YYYY-MM-DD")
    parser.add_argument("--intervals", default="5,15", help="봉 간격 (쉼표 구분)")
    ...
    print(f"\n{'='*70}")
    print(f"  단타 전략 파라미터 최적화")
    print(f"  Symbol: {args.symbol} | 기간: {args.start} → {args.end}")
    print(f"  전략: {', '.join(strategy_names)} | 인터벌: {', '.join(intervals)}")
    print(f"{'='*70}")

    fetcher = HistoricalDataFetcher(client)
    data_cache: dict[str, tuple] = {}  # interval → (candles, funding)
    ...
    all_combos: list[tuple[str, dict, dict]] = []
        ...
        for interval in intervals:
            for jcfg in STRATEGY_GRIDS[strat_name]:
                ...
                for defense in DEFENSE_GRID:
                    all_combos.append((interval, cfg, defense))
    ...
    # ── Step 3: Run all backtests (sync, no API calls) ──
    results: list[dict] = []
    start_time = time.time()

    for i, (interval, jcfg, defense) in enumerate(all_combos):
        candles, funding = data_cache[interval]
        r = run_single(candles, funding, jcfg, defense)
        if r is not None:
            r["interval"] = interval
            results.append(r)
    ...
    # ── Step 4: Rank results ──
    # Primary: net_pnl, Secondary: sharpe, Tertiary: lower drawdown
    ...
    for i, r in enumerate(top, 1):
        ...
        defense = r["defense"]
        print(f"\n{'─'*70}")
        print(f"  #{i}  [{module}] {r['interval']}분봉")
        print(f"{'─'*70}")
    ...
    bot_config = {
        "name": f"최적화_{best['judgment']['module']}_{best['interval']}m",
        "symbol": args.symbol,
        "category": "linear",
        "seed_budget": 10000,
        "leverage": 1,
